...
AI agents write code fast. They also *silently remove logic*, change behavior, and introduce bugs — without telling you. You find out in production.
...
2. **`git add .` → `lrc review`** — AI flags issues
3. **Copy issues, feed them back** to your agent to fix
4. **`git add .` → `lrc review`** — AI reviews again
5. Repeat until satisfied
6. **`lrc review --vouch`** → **`git commit`** — you vouch and commit

Each `lrc review` is an **iteration**. The tool tracks how many iterations you did and what percentage of the diff was AI-reviewed (**coverage**).
